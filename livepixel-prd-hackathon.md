...
3. **Prompt Interpreter (Text → Spec) — Autoload: GeminiAPI**
   - Routes to `gemini-3.1-flash-preview` for simple parameter changes
...
- **Lightweight Task LLM: `gemini-3.1-flash-preview`**
...
- Automatically routes to the appropriate model based on request type from frontend/backend:
  - **Minor modifications during real-time play** → `gemini-3.1-flash-preview`
